...
This document is the contract between the upstream `genDiv` pipeline and the downstream GPA consumer for Step 7 of the refactor. It covers:
...
The original proposal mentioned `freqs.${rg}.tab.gz` + `.tbi` as a tabix-indexed AF file for `bcftools roh`. The upstream pipeline instead runs `bcftools roh --estimate-AF -` on the group-subset VCF, which computes group AF internally. **No `freqs.*.tab.gz` files exist.** If GPA reads these anywhere, remove those reads — they were never wired up.
...
2. **Update the copy block** in `create_popFiles.sh` to iterate over `rg ∈ {wholePop, Trotter, Pacer}` and copy the eight files listed in §1's table. Template:
   ```bash
...
4. **Drop any `freqs.*.tab.gz` copies** if present — upstream no longer produces those.
5. **Add a validator call** at the end of `create_popFiles.sh` (see §5 below).
6. **Update the report generator** (`generate_report.py` and the `plot_*.R` set in `../GPA/`) to read `${rg}.` suffixed filenames for all three tabs. Columns indices unchanged; only filenames change.
7. **Regenerate test fixtures** if GPA has recorded-output tests pinned to the old bare-named files.
8. **Before merge:** run a full GPA report end-to-end on a populated `popFiles/` dir and confirm each tab (wholePop, Trotter, Pacer) renders with a distinct distribution for F_SNP, F_ROH, D_SNP, G_SNP, D_ROH, G_ROH, and ROH_sh. (The wholePop tab should look like today's single report; the other two should be new.)
