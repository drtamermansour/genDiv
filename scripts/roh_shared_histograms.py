...
def create_roh_shared_histograms(file1_path, file2_path, output_path="roh_shared_histograms.png"):
    """
    Create histograms of Percent_of_Consensus_ROH 
    ...
    for book_size in book_size_order:
        ...
        if len(subset) > 0:
            hist_data.append(subset['Percent_of_Consensus_ROH'])
    ...
    if hist_data:
        ...
        for i, (book_size, color) in enumerate(zip(book_size_order[:len(hist_data)], [colors[bs] for bs in book_size_order[:len(hist_data)]])):
            subset = pacer_data[pacer_data['book_size'] == book_size]
            if len(subset) > 0:
                mean_val = subset['Percent_of_Consensus_ROH'].mean()
    ...
    if not pacer_data.empty:
        stats_text = f'Overall Mean: {pacer_data["Percent_of_Consensus_ROH"].mean():.1f}\n'
        stats_text += f'Overall Std: {pacer_data["Percent_of_Consensus_ROH"].std():.1f}\n'
        stats_text += f'Overall Median: {pacer_data["Percent_of_Consensus_ROH"].median():.1f}\n'
        stats_text += f'Min: {pacer_data["Percent_of_Consensus_ROH"].min():.1f}, Max: {pacer_data["Percent_of_Consensus_ROH"].max():.1f}'
    ...
    if not trotter_data.empty:
        stats_text = f'Overall Mean: {trotter_data["Percent_of_Consensus_ROH"].mean():.1f}\n'
        stats_text += f'Overall Std: {trotter_data["Percent_of_Consensus_ROH"].std():.1f}\n'
        stats_text += f'Overall Median: {trotter_data["Percent_of_Consensus_ROH"].median():.1f}\n'
        stats_text += f'Min: {trotter_data["Percent_of_Consensus_ROH"].min():.1f}, Max: {trotter_data["Percent_of_Consensus_ROH"].max():.1f}'
    ...
    print("\n" + "="*60)
    print("COMPREHENSIVE STATISTICS: ROH_shared")
    print("="*60)
    ...
    for gait in ['Pacer', 'Trotter']:
        ...
        if not gait_data.empty:
            ...
            print(f"  Mean: {gait_data['Percent_of_Consensus_ROH'].mean():.2f}")
            print(f"  Median: {gait_data['Percent_of_Consensus_ROH'].median():.2f}")
            print(f"  Std: {gait_data['Percent_of_Consensus_ROH'].std():.2f}")
            print(f"  Range: [{gait_data['Percent_of_Consensus_ROH'].min():.2f}, {gait_data['Percent_of_Consensus_ROH'].max():.2f}]")
            ...
            for book_size in book_size_order:
                ...
                if len(subset) > 0:
                    print(f"    {book_size} (n={len(subset)}): "
                          f"Mean={subset['Percent_of_Consensus_ROH'].mean():.2f}, "
                          f"Median={subset['Percent_of_Consensus_ROH'].median():.2f}")
...
def main():
    ...
    df = create_roh_shared_histograms(file1, file2, outHisto)
