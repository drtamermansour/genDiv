...
class ROHAnalysis:
    ...
    def analyze_subpopulations(self):
        ...
        # Finalize Plot
        plt.title("Genome-Wide ROH Frequency")
        plt.xlabel("SNP Index (Genome Order)")
        plt.ylabel("Frequency of ROH")
        plt.legend()
        plt.tight_layout()
        plt.savefig(f"{output_dir}/ROH_Frequency_Plot.{roh_mb_cutoff}.png")
        print(f"\nPlot saved to '{output_dir}/ROH_Frequency_Plot.{roh_mb_cutoff}.png'")
        
        # Save Stats
        pd.DataFrame(results_stats).to_csv(f"{output_dir}/ROH_Subpop_Stats.{roh_mb_cutoff}.csv", index=False)
        print(f"Stats saved to '{output_dir}/ROH_Subpop_Stats.{roh_mb_cutoff}.csv'")

        # Save ISLANDS Table (The detailed regions)
        islands_df = pd.DataFrame(all_islands)
        islands_df.to_csv(f"{output_dir}/ROH_Islands_Detailed.{roh_mb_cutoff}.csv", index=False)
        print(f"A List of detected genomic regions saved to '{output_dir}/ROH_Islands_Detailed.{roh_mb_cutoff}.csv'")
